# Optional CAD Architecture — V0.6

## Scope

V0.6 proves one narrow dependency path without redesigning the V0.5 viewer or claiming a complete shed CAD model:
...
`mbse_lite.geometry`, `mbse_lite.cad.protocol` and `mbse_lite.cad.runner` have no CadQuery dependency. The main process validates the model and serializes a job; only `mbse_lite.cad.worker` imports `mbse_lite.cad.cadquery_backend`, and only that backend imports CadQuery. Importing MBSE Lite, importing the public CAD package, or invoking any CLI command never loads CadQuery/OCP into the main process.
...
Each direct CadQuery assembly child is named with its mapped stable PART ID. Candidate metadata still comes only from the explicit `mower_door_candidate` and `main_door_candidate` roles. Adding an unrelated preferred Concept cannot affect the assembly component set.
...
- requested in-memory component names and identity observed after STEP/GLB export.
...
- verifies all seven stable component IDs survived STEP re-import and GLB node export;
...
The parent removes any old completion record before launch, then validates the new record's schema and exact job ID. It independently checks each recorded file and size, the requested footprint extent, manifest authority/scope/provenance, backend metadata and identity evidence. A stale record, a record written before the current job, a missing/empty/changed artifact, or a failed semantic check cannot establish success.
...
## Stable identity investigation

With CadQuery 2.8.0/OCP 7.9.3.1.1, the seven direct PART names survive semantic STEP assembly re-import. They also appear as exact GLB node names. The exported GLB does not contain `node.extras.mbse_id`, which is the stronger selection contract used by the current offline Three.js viewer.

The result is therefore suitable evidence for future integration, but not a drop-in viewer replacement. V0.6 leaves `mbse-lite view`, its deterministic custom `views/model.glb`, and its `extras.mbse_id` selection behavior unchanged. A later version can choose either a small validated metadata addition or an explicitly documented node-name fallback after cross-platform exporter testing.
