...
Start V0.10 editable mode on the loopback-only default URL `http://127.0.0.1:8000/`:
...
V0.10 uses Python's small standard-library HTTP server behind `ServeApplication`, so no server package or `serve` extra is required. The server accepts only loopback binding, serves a fixed viewer and packaged browser assets, rejects arbitrary paths and cross-origin writes, requires strict JSON, and limits write bodies to 64 KiB. It is a single-user local engineering application, not a remotely deployable service.
...
V0.10 adds one deliberately narrow authoring operation: **Create Requirement**. The server discovers the single existing Markdown table that owns Requirement objects and advertises its actual non-ID columns, a deterministic table revision, and a non-authoritative next-ID suggestion. The browser builds the form from that metadata and sends only `values` plus `expected_table_revision` to `POST /api/requirements`; it cannot choose the ID, file, table, or row. `CreateRequirement` reloads the current project under the shared mutation lock, rejects stale revisions, allocates one `REQ-*` ID as the greatest current numeric suffix plus one (minimum three-digit padding), encodes one safe Markdown row, validates a disposable candidate, atomically commits, reloads, verifies provenance, and returns the created ID with a fresh snapshot. A new Requirement is selected immediately and remains unconnected until relations are authored in a future milestone.

Only Requirements can be created. V0.10 does not create other object types or relations, delete or duplicate objects, edit general Markdown, or trigger CAD. If there is no existing writable Requirement table, or more than one candidate table, creation is disabled with a reason rather than guessing. A table change after the form opens returns HTTP 409; the UI retains the entered values while refreshing the authoritative project and revision.
...
See `docs/VIEW_ARCHITECTURE.md` for the V0.10 static/editable viewer architecture, retained V0.5 manifest schema, source provenance, controlled Requirement creation, offline packaging, optional CAD boundary, SVG/glTF identity, binary transport, renderer lifecycle and object-selection contracts.
